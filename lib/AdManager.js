...
var AdManager = new function() {
	...
	*/
	...
	self.parseJson = function(_rawJsonData, returnIndex) {
		...
		if (self.jsonMode === 'timeslot') jsonData = [self.getAdDataFromTimeslot(jsonData)]
		...
		if (hasExpiration) dateSortedAdArray = adDataArray.sort(sortByExpirationDate)
	...
	self.getAdDataFromTimeslot = function(rawTimeslotData) {
		...
		if (self.adManagerSettings.includeJsonDataByKey.key.length !== 0) {
			...
			if (adDataArray === 'pending' || !adDataArray) return adDataArray
				...
				if (expiration_time.getTime() < self.getNow().getTime()) {
					console.log(
						' - ad "' +
							adsArray[k].name +
							'" expired: ' +
							adsArray[k].expiration_time +
							', distributing ' +
							adsArray[k].weight +
							'% of weight to other ads'
					)
	...
	self.includeAdDataByKey = function(adArray, type) {
		...
		console.log(
			' - Searching feed for a key: "' +
				self.adManagerSettings.includeJsonDataByKey.key +
				'" ' +
				'with values matching:' +
				self.adManagerSettings.includeJsonDataByKey.values
					.map(function(value) {
						return '"' + value + '"'
					})
					.join(', ')
		...
		if (adArray[self.currentAdDataIndex] !== undefined) {
			if (type === 'timeslots') ads = adArray[self.currentAdDataIndex].ads
			else if (type === 'ads') ads = adArray
		} else return false
		...
		for (var i in ads) {
			...
			var jsonScope = ads[i]
			for (var k in keys) {
				if (keys[k] in jsonScope) jsonScope = jsonScope[keys[k]]
				else {
			...
			if (self.adManagerSettings.excludeJsonDataByKey.key in adData) {
				for (var j = 0; j < self.adManagerSettings.excludeJsonDataByKey.values.length; j++) {
					if (adData[self.adManagerSettings.excludeJsonDataByKey.key] == self.adManagerSettings.excludeJsonDataByKey.values[j]) {
						console.log(
							'AdManager.hasExcludedKeyValue(), "' +
								self.adManagerSettings.excludeJsonDataByKey.key +
								'": "' +
								self.adManagerSettings.excludeJsonDataByKey.values[j] +
								'"'
						)
						return true
					}
