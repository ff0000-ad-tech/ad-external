...
import { DateManager as DM } from 'ad-dates'
...
class Velvet {
	...
	*/
	init(settings){
		...
		trace ( '\tbase url:', V._baseUrlNow )
		trace ( '\tbase slugs:', V._baseSlugs )
	...
	*/
	get(){
		...
		if ( _pathed ){
			...
			if ( V._debug ) trace ( '\tanchor:', anchor, '\n\tsplits again:', splits )
		}		
		...
		function walkObject ( obj ){
			if ( V._debug ) trace ( 'walkObject() >', obj )
		...
		function readProperty ( obj, param ){
			if ( V._debug ) trace ( '\t\t\t readProperty()', obj, param, '|', anchor, '|', targetKey )
	...
	*/
	...
	/* ------------------------------------------------------------------------------------------------------------- */
	// EVENT HANDLERS	
	_handleSegmentLoadComplete(event) {
		const V = this
		...
		trace ( '\tselected ad slug:', V._settings.adData )

		V._loadAdData();
	}

	_handleAdDataLoadComplete(event) {
		const V = this
		trace ( 'Velvet._handleAdDataLoadComplete()' )
